...
export default {
  ...
  methods: {
    findInputmaterial() {
      this.$http.get(`/inputmaterial/${this.$route.params.id}`)
        .then((res) => {
          ...
          this.baseFields = this.inputmaterial;
        })
        .catch((err) => {
    ...
    edit() {
      ...
      this.$http.patch(`/inputmaterial/${this.inputmaterial.id}`, this.baseFields)
        .then((res) => {
          window.console.log(res);
          this.$store.dispatch('snackbar/success', 'Registro editado éxitosamente');
          this.$router.push({ name: 'inputmaterial.show', params: { id: this.inputmaterial.id } });
        })
        .catch((err) => {
          this.$store.dispatch('snackbar/error', statuses[err.response.status]);
        });
