<template>
  <v-layout row wrap>
   <v-flex xs12 class="mb-3 offset-xs9">
      <v-btn color="info" :to="{ name: 'tool.create' }"> Registrar nuevo </v-btn>
    </v-flex>
            ...
            <template v-slot:item.action="{ item }">
              <v-tooltip bottom color="info">
                <template v-slot:activator="{ on }">
                  <v-btn icon small text :to="`/tool/show/${item.id}`"
                  >
              ...
              <v-tooltip bottom color="success">
                <template v-slot:activator="{ on }">
                  <v-btn icon small text :to="`/tool/edit/${item.id}`"
                  >
...
export default {
  ...
  methods: {
    ...
    getTool() {
      ...
      this.$http.get('/tool')
        .then((res) => {
          ...
          this.registers = res.data.tools;
          this.warehouse = res.data.tools.warehouse;
          this.loading = false;
        })
        .catch((err) => {
    ...
    deleteRegister() {
      this.$http.delete(`/tool/${this.deleteDialogRegister.id}`)
        .then((res) => {
