<template>
  <v-layout row wrap class="px-3">
    ...
    <v-flex xs6 class="offset-xs3 mb-3">
      <v-card>
        <v-card-title>Modificar Actividad</v-card-title>
          ...
          <v-form @submit.prevent>
            <v-container fluid grid-list-xs>
              ...
              <v-layout row wrap>
                ...
                <v-flex xs12>
                  <v-text-field v-model="baseFields.status"  append-icon="mdi-card" label="Estado" outlined
                    required
                  ></v-text-field>
                </v-flex>
              </v-layout>
...
export default {
  ...
  methods: {
    ...
    edit() {
      ...
      this.$http.patch(`/activity/${this.activity.id}`, this.baseFields)
        .then((res) => {
          window.console.log(res);
          this.$store.dispatch('snackbar/success', 'Actividad editada éxitosamente');
          this.$router.push({ name: 'activity.show', params: { id: this.activity.id } });
        })
        .catch((err) => {
          this.$store.dispatch('snackbar/error', statuses[err.response.status]);
        });
